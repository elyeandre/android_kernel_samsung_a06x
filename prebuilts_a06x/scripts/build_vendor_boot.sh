...
setup_boot_editor() {
    ...
    # boot_editor's PackableLauncher uses File("vendor_boot.img").exists() for
    # handler detection — it must be present at the root even for pack operations.
    # pack() reads all params from build/unzip_boot/vendor_boot.json, not this file.
    touch "${BOOT_EDITOR_DIR}/vendor_boot.img"
